...
def cr_inter_mode(net, leaves, dtr, ris):
  while True:
    ...
    if cr == "quit":
      ...
      crf = pnml2nf(crf, dtr) 
      startcr = time.time()
      ans = comp_1rev(net, crf, leaves)
      endcr = time.time()
      ris.write("Time for collective reveals: " + str(endcr - startcr) + '\n')
      if ans == 0:
          print("The set "+ str(crf[0]) + " "+ str(crf[2]) + "-collective reveals " + str(crf[1])) 
      else:
          print("The set "+ str(crf[0]) + " does NOT "+ str(crf[2]) + "-collective reveals " + str(crf[1])) 
...
#  global nodi
  ...
  if fc == False:
      ...
      if args:
        ...
        ris.write(fn + '-full tree\n')
        qr = ([], [], 1)
      start = time.time()
      adj = redmsmg(net, qr[1])
      end = time.time()     
      ris.write("Time for the marking graph: " + str(end - start) + '\n')
      print("Time for the marking graph: " + str(end - start) + '\n')
      ris.write("Number of markings: " + str(len(adj)) + '\n')
      print("Number of markings: " + str(len(adj)) + '\n')
      ...
      endt = time.time()
      ris.write("Time for the tree: " + str(endt - startt) + '\n')
      print("Time for the tree: " + str(endt - startt) + '\n')
      ris.write("Leaves: " + str(len(leaves)) + '\n')
      ...
      if al == True:
        ...
        enda = time.time()
        ris.write('Time with option -a: ' + str(enda - starta) + '\n')
        ...
        cr_inter_mode(net, leaves, dtr, ris)
      if args or fr:
        startcr = time.time()
        ans = comp_1rev(net, qr, leaves)
        endcr = time.time()
        ris.write("Time for collective reveals: " + str(endcr - startcr) + '\n')
        if ans == 0:
